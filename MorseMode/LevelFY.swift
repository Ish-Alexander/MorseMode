...
struct LevelFY: View {
    ...
    private func createNewItem() -> String {
        ...
        let newLetter = remainingLetters.randomElement() ?? targetLetters.randomElement() ?? "F"
        letter = newLetter
    ...
    private func playSoundForCurrentLetter() {
        ...
        guard let url = foundURL else {
            ...
            audioPlayer = nil
            print("[Audio][LevelFY] No audio file found for letter \(first)")
            return
        }
        ...
        do {
            ...
            print("[Audio][LevelFY] Failed to play \(url.lastPathComponent): \(error)")
        }
    }
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 14) {
                ...
                Text("Listen, then tap the Morse code for F or Y.")
                    .font(.custom("berkelium bitmap", size: 12))
                    .foregroundStyle(Color.white.opacity(0.82))
                    .multilineTextAlignment(.center)

                HStack(spacing: 18) {
                    Text("F: \(correctCounts["F", default: 0])/5")
                    Text("Y: \(correctCounts["Y", default: 0])/5")
                }
                .foregroundStyle(.neon)
                .font(.custom("berkelium bitmap", size: 14))

                Text("Letters in play: F  Y")
                    .font(.custom("berkelium bitmap", size: 12))
