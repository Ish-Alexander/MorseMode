...
struct LevelRK: View {
    ...
    private func createNewItem() -> String {
        ...
        let newLetter = remainingLetters.randomElement() ?? targetLetters.randomElement() ?? "R"
        letter = newLetter
    ...
    private func playSoundForCurrentLetter() {
        ...
        guard let url = foundURL else {
            ...
            audioPlayer = nil
            print("[Audio][LevelRK] No audio file found for letter \(first)")
            return
        }
        ...
        do {
            ...
            print("[Audio][LevelRK] Failed to play \(url.lastPathComponent): \(error)")
        }
    }
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 14) {
                ...
                Text("Listen, then tap the Morse code for R or K.")
                    .font(.custom("berkelium bitmap", size: 12))
                    .foregroundStyle(Color.white.opacity(0.82))
                    .multilineTextAlignment(.center)

                HStack(spacing: 18) {
                    Text("R: \(correctCounts["R", default: 0])/5")
                    Text("K: \(correctCounts["K", default: 0])/5")
                }
                .foregroundStyle(.neon)
                .font(.custom("berkelium bitmap", size: 14))

                Text("Letters in play: R  K")
                    .font(.custom("berkelium bitmap", size: 12))
