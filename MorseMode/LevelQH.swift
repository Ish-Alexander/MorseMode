...
struct LevelQH: View {
    ...
    private func createNewItem() -> String {
        ...
        let newLetter = remainingLetters.randomElement() ?? targetLetters.randomElement() ?? "Q"
        letter = newLetter
    ...
    private func playSoundForCurrentLetter() {
        ...
        guard let url = foundURL else {
            ...
            audioPlayer = nil
            print("[Audio][LevelQH] No audio file found for letter \(first)")
            return
        }
        ...
        do {
            ...
            print("[Audio][LevelQH] Failed to play \(url.lastPathComponent): \(error)")
        }
    }
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 14) {
                ...
                Text("Listen, then tap the Morse code for Q or H.")
                    .font(.custom("berkelium bitmap", size: 12))
                    .foregroundStyle(Color.white.opacity(0.82))
                    .multilineTextAlignment(.center)

                HStack(spacing: 18) {
                    Text("Q: \(correctCounts["Q", default: 0])/5")
                    Text("H: \(correctCounts["H", default: 0])/5")
                }
                .foregroundStyle(.neon)
                .font(.custom("berkelium bitmap", size: 14))

                Text("Letters in play: Q  H")
                    .font(.custom("berkelium bitmap", size: 12))
