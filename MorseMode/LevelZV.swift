...
struct LevelZV: View {
    ...
    private func createNewItem() -> String {
        ...
        let newLetter = remainingLetters.randomElement() ?? targetLetters.randomElement() ?? "Z"
        letter = newLetter
    ...
    private func playSoundForCurrentLetter() {
        ...
        guard let url = foundURL else {
            ...
            audioPlayer = nil
            print("[Audio][LevelZV] No audio file found for letter \(first)")
            return
        }
        ...
        do {
            ...
            print("[Audio][LevelZV] Failed to play \(url.lastPathComponent): \(error)")
        }
    }
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 14) {
                ...
                Text("Listen, then tap the Morse code for Z or V.")
                    .font(.custom("berkelium bitmap", size: 12))
                    .foregroundStyle(Color.white.opacity(0.82))
                    .multilineTextAlignment(.center)

                HStack(spacing: 18) {
                    Text("Z: \(correctCounts["Z", default: 0])/5")
                    Text("V: \(correctCounts["V", default: 0])/5")
                }
                .foregroundStyle(.neon)
                .font(.custom("berkelium bitmap", size: 14))

                Text("Letters in play: Z  V")
                    .font(.custom("berkelium bitmap", size: 12))
