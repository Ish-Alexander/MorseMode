...
struct LevelDU: View {
    ...
    private func createNewItem() -> String {
        ...
        let newLetter = remainingLetters.randomElement() ?? targetLetters.randomElement() ?? "D"
        letter = newLetter
    ...
    private func playSoundForCurrentLetter() {
        ...
        guard let url = foundURL else {
            ...
            audioPlayer = nil
            print("[Audio][LevelDU] No audio file found for letter \(first)")
            return
        }
        ...
        do {
            ...
            print("[Audio][LevelDU] Failed to play \(url.lastPathComponent): \(error)")
        }
    }
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 14) {
                ...
                Text("Listen, then tap the Morse code for D or U.")
                    .font(.custom("berkelium bitmap", size: 12))
                    .foregroundStyle(Color.white.opacity(0.82))
                    .multilineTextAlignment(.center)

                HStack(spacing: 18) {
                    Text("D: \(correctCounts["D", default: 0])/5")
                    Text("U: \(correctCounts["U", default: 0])/5")
                }
                .foregroundStyle(.neon)
                .font(.custom("berkelium bitmap", size: 14))

                Text("Letters in play: D  U")
                    .font(.custom("berkelium bitmap", size: 12))
